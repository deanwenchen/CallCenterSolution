...
- **Global scope**: `D:/Claude/CallCenterSolution1/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Claude/CallCenterSolution1/.claude/notes/`)
...
2. Glob `D:/Claude/CallCenterSolution1/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Claude/CallCenterSolution1/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Claude/CallCenterSolution1/.claude/notes/` — works in any directory
